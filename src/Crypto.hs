...
generatePasswordHash pass salt =
    let
        passWithSalt = TEnc.encodeUtf8 $ pass <> salt
        hashed = DBE.convertToBase DBE.Base16 $ Hash.hashWith Hash.SHA256 passWithSalt
     in TEnc.decodeUtf8 hashed

validatePassword :: T.Text -> T.Text -> Bool
validatePassword pass passHashWithSalt =
    let
        (passHash, salt) = splitAtEnd (saltLength*2) passHashWithSalt
     in case salt of
        "" -> False
